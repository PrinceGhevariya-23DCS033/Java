...
// Demonstrate character stream 
        try (FileReader fr = new FileReader("input.txt"); FileWriter fw = new FileWriter("output_char.txt")) {
            int c;
            while ((c = fr.read()) != -1) {
                fw.write(c);
            }
...
// Demonstrate byte stream 
        try (FileInputStream fis = new FileInputStream("input.txt"); FileOutputStream fos = new FileOutputStream("output_byte.txt")) {
...
// Use BufferedReader and BufferedWriter to read from console and write to a file 
        try (BufferedReader br = new BufferedReader(new InputStreamReader(System.in)); BufferedWriter bw = new BufferedWriter(new FileWriter("console_output.txt"))) {
